#!/usr/bin/env python3
"""
Jira adapter: create issues, add comments, basic search.

Auth:
  - Cloud Jira: Basic auth with email + API token.
  - Provide env: JIRA_BASE_URL, JIRA_EMAIL, JIRA_API_TOKEN (or pass explicitly).

Usage:
  from adapters.jira_adapter import Jira
  j = Jira()  # uses env by default
  j.create_issue("ENG", "Flaky test detected", "Details...")
"""
...
class Jira:
    ...
    def search(self, jql: str, max_results: int = 50) -> Dict[str, Any]:
        ...
        r = self.s.get(url, params={"jql": jql, "maxResults": max_results}, timeout=TIMEOUT)
        r.raise_for_status()
        return r.json()
